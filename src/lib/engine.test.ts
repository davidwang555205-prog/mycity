...
describe("controlled sampler", () => {
 ...
 it("gives every question distinct, situation-specific answer copy",()=>{ const all=QUESTIONS.flatMap(q=>q.options.map(o=>o.text)); expect(new Set(all).size).toBe(96*4); for(const question of QUESTIONS) expect(new Set(question.options.map(o=>o.text)).size).toBe(4); });
});
